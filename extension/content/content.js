...
(() => {
  ...
  async function startExtraction(dateFilter, windowMeta) {
    ...
    try {
      ...
      // Keep only share-type items, then reverse to chronological order so the
      // first send of any shortcode is its oldest occurrence.
      let shares = items
        .map(item => ChatParser.parseShare(item, myUserId, userMap))
        .filter(Boolean);
      shares.reverse();

      // Apply client-side date filter (operates on seconds, matching extractor).
      if (dateFilter) {
        shares = shares.filter(s => {
          const ts = s.timestampUnix;
          return ts >= dateFilter.startUnix && ts <= dateFilter.endUnix;
        });
      }

      // Total thread items scanned (all messages fetched, before share filtering).
      const messagesScanned = items.length;

      // Build the categorized shares JSON output.
      const jsonData = ChatParser.buildCategorizedOutput(threadInfo, shares, myUserId, userMap, {
        window: windowMeta || null,
        messagesScanned,
      });
      state.jsonData = jsonData;
      state.chatTitle = jsonData.source.chatWith;
      state.summary = jsonData.summary;

      state.status = 'complete';
      state.totalMessages = jsonData.summary.uniqueShares;
      ...
      browserAPI.runtime.sendMessage({
        type: 'UPDATE_BADGE',
        count: jsonData.summary.uniqueShares,
      }).catch(() => {});
    } catch (err) {
      state.status = 'error';
  ...
  function handleMessage(msg) {
    switch (msg.type) {
      ...
      case 'START_EXTRACTION': {
        ...
        startExtraction(msg.dateFilter || null, msg.window || null);
        return resp;
      }
